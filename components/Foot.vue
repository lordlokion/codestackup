<template>
  <div class="max-w-6xl mx-auto flex flex-col md:flex-row">
    <!-- Contact Section -->
    <div
      class="w-full md:w-1/3 rounded-2xl h-60 mb-10 py-4 px-2"
      style="background: linear-gradient(to bottom, #e8f0d2, #ffffff)"
    >
      <div class="mb-4"><img src="/icon/logo_black.svg" class="" /></div>
      <div class="flex flex-col md:flex-row">
        <div class="flex w-full md:w-1/3 mt-6 mx-4">
          <img src="/images/email.png" class="h-24 w-24 object-cover" />
        </div>
        <div class="w-full md:w-1/2 pt-8">
          <ul class="text-rhino-400 text-sm">
            <li>Contact: hello@codestackup.com</li>
            <li>Support: help@codestackup.com</li>
          </ul>
        </div>
      </div>
    </div>

    <!-- Menu Section -->
    <div class="hidden md:w-1/3 md:flex md:flex-col border-r-2 border-lime-600">
      <div class="text-center font-bold text-xl mb-8">MENU</div>
      <div class="flex flex-col md:flex-row">
        <div class="mx-auto">
          <ul>
            <NuxtLink to="/"><li>Home</li></NuxtLink>
            <NuxtLink to="/service"><li>Services</li></NuxtLink>
            <NuxtLink to="/about"><li>About</li></NuxtLink>
          </ul>
        </div>
        <div class="mx-auto mt-4 md:mt-0">
          <ul>
            <NuxtLink to="/ourprocess"><li>Our Process</li></NuxtLink>
            <NuxtLink to="/portfolio"> <li>Portfolio</li></NuxtLink>
          </ul>
        </div>
      </div>
    </div>

    <!-- Social Media Section -->
    <div class="w-full md:w-1/3">
      <div class="text-center font-bold text-xl m-8">OUR SOCIAL MEDIAS</div>
      <div class="flex flex-col md:flex-row gap-4">
        <div class="mx-auto">
          <div class="flex gap-4">
            <!-- Facebook Icon -->
            <a
              class="border-rhino-200 flex h-12 w-12 items-center justify-center rounded-full border text-gray-600 transition duration-200 hover:border-transparent hover:bg-lime-600 hover:text-white"
              href="#"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  d="M13.6348 20.7273V12.766H16.3582L16.7668 9.66246H13.6348V7.68129C13.6348 6.78302 13.8881 6.17086 15.2029 6.17086L16.877 6.17018V3.39425C16.5875 3.35734 15.5937 3.27274 14.437 3.27274C12.0216 3.27274 10.368 4.71881 10.368 7.37392V9.66246H7.63635V12.766H10.368V20.7273H13.6348Z"
                  fill="currentColor"
                ></path>
              </svg>
            </a>

            <!-- Twitter Icon -->
            <a
              class="border-rhino-200 flex h-12 w-12 items-center justify-center rounded-full border text-gray-600 transition duration-200 hover:border-transparent hover:bg-lime-600 hover:text-white"
              href="https://twitter.com/codestackup"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  d="M21.8182 6.14598C21.1356 6.44844 20.4032 6.65356 19.6337 6.74513C20.4194 6.27462 21.0209 5.52831 21.3059 4.64177C20.5689 5.0775 19.7553 5.39389 18.8885 5.56541C18.1943 4.82489 17.207 4.36365 16.1118 4.36365C14.0108 4.36365 12.3073 6.06719 12.3073 8.16707C12.3073 8.46489 12.3409 8.75577 12.4058 9.03392C9.24437 8.87513 6.44107 7.3605 4.56486 5.05895C4.23689 5.61986 4.05031 6.27344 4.05031 6.9711C4.05031 8.29107 4.72246 9.45574 5.74228 10.1371C5.1188 10.1163 4.5324 9.94477 4.01904 9.65968V9.70719C4.01904 11.5498 5.33089 13.0876 7.07034 13.4376C6.75164 13.5234 6.41558 13.5709 6.06792 13.5709C5.82225 13.5709 5.58468 13.5466 5.35174 13.5002C5.83613 15.0125 7.24071 16.1123 8.90486 16.1424C7.60343 17.1623 5.96246 17.7683 4.18013 17.7683C3.87304 17.7683 3.57055 17.7498 3.27274 17.7162C4.95658 18.7974 6.95564 19.4279 9.10419 19.4279C16.1026 19.4279 19.9281 13.6312 19.9281 8.60398L19.9153 8.11147C20.6628 7.57834 21.3094 6.90853 21.8182 6.14598Z"
                  fill="currentColor"
                ></path>
              </svg>
            </a>

            <!-- LinkedIn Icon -->
            <a
              class="border-rhino-200 flex h-12 w-12 items-center justify-center rounded-full border text-gray-600 transition duration-200 hover:border-transparent hover:bg-lime-600 hover:text-white"
              href="https://www.linkedin.com/company/codestackup/?viewAsMember=true"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
              >
                <path
                  d="M16.2 0H1.8C0.81 0 0 0.81 0 1.8V16.2C0 17.19 0.81 18 1.8 18H16.2C17.19 18 18 17.19 18 16.2V1.8C18 0.81 17.19 0 16.2 0ZM5.4 15.3H2.7V7.2H5.4V15.3ZM4.05 5.67C3.15 5.67 2.43 4.95 2.43 4.05C2.43 3.15 3.15 2.43 4.05 2.43C4.95 2.43 5.67 3.15 5.67 4.05C5.67 4.95 4.95 5.67 4.05 5.67ZM15.3 15.3H12.6V10.53C12.6 9.81004 11.97 9.18 11.25 9.18C10.53 9.18 9.9 9.81004 9.9 10.53V15.3H7.2V7.2H9.9V8.28C10.35 7.56 11.34 7.02 12.15 7.02C13.86 7.02 15.3 8.46 15.3 10.17V15.3Z"
                  fill="currentColor"
                ></path>
              </svg>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div
    class="flex items-center justify-between max-w-6xl mx-auto flex flex-col md:flex-row mb-10 mt-8"
  >
    <p class="text-rhino-400 text-sm mb-4 md:mb-0">
      © Copyright 2023. All Rights reserved by CodeStackUp.
    </p>
    <div class="flex gap-5">
      <img
        class="h-4"
        src="https://shuffle.dev/coleos-assets/logos/logo-visa.svg"
        alt=""
      />
      <img
        class="h-4"
        src="https://shuffle.dev/coleos-assets/logos/logo-mastercard.svg"
        alt=""
      />
      <img
        class="h-4"
        src="https://shuffle.dev/coleos-assets/logos/logo-paypal.svg"
        alt=""
      />
    </div>
  </div>
</template>
